...
function move(delta){if(view==='index'||view==='proyek'||dialog.open||menu.open)return;target+=delta;wake();}
...
function openSheet(index,button){if(suppress)return;opener=button;const from=button.getBoundingClientRect();loadSheet(index);dialog.showModal();requestAnimationFrame(()=>flip(from,$('#sheet-viewport').getBoundingClientRect(),image.src));}
...
$('#menu-toggle').onclick=()=>{menu.showModal();$('#menu-toggle').setAttribute('aria-expanded','true');};$('#menu-close').onclick=closeMenu;
...
document.querySelectorAll('[data-jump]').forEach(b=>b.onclick=()=>jump(+b.dataset.jump));$('#previous').onclick=()=>move(-step);$('#next').onclick=()=>move(step);
$('#category').onchange=e=>cards.forEach(c=>c.hidden=e.target.value!=='Semua'&&c.dataset.category!==e.target.value);
stage.addEventListener('wheel',e=>{if(view==='index'||view==='proyek'||e.ctrlKey)return;e.preventDefault();move((Math.abs(e.deltaX)>Math.abs(e.deltaY)?e.deltaX:e.deltaY)*(e.deltaMode===1?18:e.deltaMode===2?innerWidth:1));},{passive:false});
...
addEventListener('keydown',e=>{if(dialog.open||menu.open||view==='index'||view==='proyek'||e.target.closest('input,select,textarea'))return;if(e.key==='ArrowRight'){e.preventDefault();move(step);}if(e.key==='ArrowLeft'){e.preventDefault();move(-step);}});
