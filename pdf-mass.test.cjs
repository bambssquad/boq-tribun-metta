...
data.r08=JSON.parse(fs.readFileSync('assets/r08/data.json','utf8'));
...
for(const scope of ['native','full','r08'])for(const thick of ['h16','h20','model'])for(const basis of ['purchase','net']){
 ...
 if(basis==='purchase')assert.equal(kg.body[0][2],String(scope==='native'?97:scope==='r08'?109:101));
...
console.log('PASS: 18 actual PDF exports follow scope/thickness/basis with matching profile and component kg/bar subtotals, R08 portrait.');
