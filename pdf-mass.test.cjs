...
data.r08=JSON.parse(fs.readFileSync('assets/r08/data.json','utf8'));data.r08_options=JSON.parse(fs.readFileSync('assets/r08/options.json','utf8'));
...
for(const [scope,structure] of [['native','baseline'],['full','baseline'],['r08','baseline'],['r08','a'],['r08','c']])for(const thick of ['h16','h20','model'])for(const basis of ['purchase','net']){
 const s={...B.defaults(),scope,structure,case:thick,shs:'s17',basis};const tables=[];
 ...
 if(basis==='purchase')assert.equal(kg.body[0][2],String(scope==='native'?97:scope==='r08'?B.r08Data(data,s).rhs.stock_count:101));
 if(scope==='r08')assert(pdf.internal.pageSize.getHeight()>pdf.internal.pageSize.getWidth());
...
console.log('PASS: 30 actual PDF exports follow scope/thickness/basis with matching profile and component kg/bar subtotals, R08 portrait.');
