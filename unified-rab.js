(()=>{
 ...
 const select=mode=>{full.hidden=mode!=='full';quantity.hidden=mode!=='quantity';document.querySelectorAll('[data-rab-mode]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.rabMode===mode)));if(mode==='full')frame.contentWindow?.postMessage({type:'metta-rab-measure'},location.origin);};
 document.querySelectorAll('[data-rab-mode]').forEach(b=>b.addEventListener('click',()=>select(b.dataset.rabMode)));
 const route=()=>{if(['#boq','#penawaran'].includes(location.hash)){select('quantity');if(location.hash==='#penawaran')document.getElementById('rincian-penawaran').open=true;}else if(['#anggaran','#fastener-rab','#penawaran-r04'].includes(location.hash))select('full');};
