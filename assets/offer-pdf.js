...
const MettaPDF=(()=>{
 ...
 table(['Rekap biaya','Nilai'],[...Object.entries(t.groups),['Biaya langsung',t.direct],['Overhead '+s.overhead+'%',t.oh],['Laba '+s.profit+'%',t.profit],['Pajak '+s.tax+'%',t.tax],['TOTAL PENAWARAN',t.total]].map(([a,b])=>[a,rp(b)]));
 ...
 if(o.detail!==false){d.addPage();y=18;text('LAMPIRAN - RINCIAN PEKERJAAN',14);table(['Kode / uraian','Volume','Sat.','Harga','Jumlah'],t.items.map(r=>[r.code+' / '+r.name+(r.supplier?'\n'+r.supplier:''),new Intl.NumberFormat('id-ID',{maximumFractionDigits:4}).format(r.qty),r.unit,rp(r.price),rp(r.amount)]));text('Total penawaran: '+rp(t.total),11);}
