(()=>{
 ...
 const select=mode=>{full.hidden=mode!=='full';quantity.hidden=mode!=='quantity';const labor=document.getElementById('rab-labor-panel');if(labor)labor.hidden=mode!=='labor';document.querySelectorAll('[data-rab-mode]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.rabMode===mode)));if(mode==='full')frame.contentWindow?.postMessage({type:'metta-rab-measure'},location.origin);};
