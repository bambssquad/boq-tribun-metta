...
if(typeof document!=='undefined'){
 ...
 let data,rows,settings,filter='Semua',totals;let groupMode='component';try{groupMode=localStorage.getItem('metta-boq-group-mode')||'component';}catch{};
 ...
 function laborDetail(r){
  if(r.code!=='U01')return '';
  const parts=totals.items.filter(x=>x.category==='Material'&&x.kg_per_unit>0);
  const auto=r.autoWeight!==false&&r.manualAmount==null;
  const amounts=parts.map(x=>Math.round(x.qty*x.kg_per_unit*r.price));if(amounts.length)amounts[amounts.length-1]=r.amount-amounts.slice(0,-1).reduce((a,b)=>a+b,0);
  const body=auto?parts.map((x,i)=>{const kg=x.qty*x.kg_per_unit;return `<tr><td>${esc(x.code+' / '+x.name)}</td><td class="num">${num(kg,4)}</td><td class="num">${rp(r.price)}</td><td class="num">${rp(amounts[i])}</td></tr>`;}).join(''):`<tr><td>${esc(r.name)}</td><td class="num">${num(r.qty,4)}</td><td class="num">${rp(r.price)}</td><td class="num">${rp(r.amount)}</td></tr>`;
  return `<tr class="labor-detail-row"><td colspan="6"><details><summary>Rincian fabrikasi & pemasangan — buka / tutup</summary><p>Tarif gabungan mencakup pemotongan, las, bor dan pemasangan. Rincian berikut memakai berat setiap komponen. Untuk memisahkan aktivitas dan tarifnya, gunakan editor upah di bawah.</p><div class="table-wrap"><table><thead><tr><th>Komponen / pekerjaan</th><th>Volume ${auto?'kg':esc(r.unit)}</th><th>Tarif satuan</th><th>Jumlah</th></tr></thead><tbody>${body}</tbody><tfoot><tr><td colspan="3">Total fabrikasi + pemasangan</td><td class="num">${rp(r.amount)}</td></tr></tfoot></table></div><p>Tekuk, cat, kayu dan bahan habis pakai dihitung pada baris terpisah. Pembulatan rupiah disesuaikan pada baris terakhir agar total rincian sama dengan item induk.</p></details></td></tr>`;
 }
 function render(){
  summary();
  const selected=totals.items.filter(r=>filter==='Semua'||filter===r.category);
  const row=r=>`<tr><td><b>${r.code}</b><br>${esc(r.name)}<span class="status">${esc(r.status)}</span></td><td class="num">${num(r.qty,3)}</td><td>${esc(r.unit)}</td><td>${r.code==='U01'?rp(r.price):`<input aria-label="Harga ${esc(r.name)}" data-price="${r.code}" type="number" min="0" step="any" value="${r.price}">`}</td><td class="num" data-amount="${r.code}">${num(r.amount)}</td><td>${esc(r.basis)}${r.supplier?`<p><b>${esc(r.supplier)}</b><br>${esc(r.supplier_contact)}<br>${esc(r.specification)}<br>${esc(r.price_basis)}</p>`:''}${data.sources[r.source]?`<br><a href="${data.sources[r.source][1]}" target="_blank" rel="noopener">Sumber / pemasok ↗</a>`:''}</td></tr>${laborDetail(r)}`;
  $('rows').innerHTML=BOQGroups.groups(selected,groupMode).map(g=>`<tr class="component-group"><td colspan="6"><details open><summary>${esc(g.name)} <span>${rp(g.amount)}</span></summary><div class="table-wrap"><table><thead><tr><th>Kode / item</th><th>Volume</th><th>Sat.</th><th>Harga satuan</th><th>Jumlah</th><th>Dasar / sumber</th></tr></thead><tbody>${g.items.map(row).join('')}</tbody></table></div></details></td></tr>`).join('');
 ...
 fetch('assets/r04/data.json').then(r=>{if(!r.ok)throw Error('load');return r.json();}).then(d=>{
  ...
  $('boq-group-mode').value=groupMode;$('boq-group-mode').addEventListener('change',e=>{groupMode=e.target.value;try{localStorage.setItem('metta-boq-group-mode',groupMode);}catch{};window.MettaOffer.get().groupMode=groupMode;render();});
  ...
  $('rab-download-action').addEventListener('click',()=>{
   ...
   const bytes=XL.book([{name:'REKAP',rows:table(rekap),cols:[58,26]},{name:'RAB',rows:table(rab),cols:[14,26,60,18,15,22,24,90,40,40,80,70,70]},{name:'PENGATURAN',rows:table(config),cols:[38,34]}]);
