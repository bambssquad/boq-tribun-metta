...
data.r08_options=read('assets/r08/options.json');
...
for(const structure of ['baseline','a','c']){
const s={...B.defaults(),scope:'r08',structure,case:'model',shs:'reference',mesh:'m20_25',extras:true,subtotals:true,componentSubtotals:true,kgExplanation:true,date:'2026-09-15'};
const pdf=B.pdf(data,s,ctx.jspdf.jsPDF);
fs.writeFileSync(`assets/r08/METTA-R08${structure==='baseline'?'':'-'+structure.toUpperCase()}-RAB-portrait.pdf`,Buffer.from(pdf.output('arraybuffer')));
console.log({structure,pages:pdf.getNumberOfPages(),...B.totals(B.rows(data,s)),kg:B.massSummary(data,s).kg});
}
