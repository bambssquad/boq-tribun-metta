...
(async()=>{
 ...
 const originalPdf=B.pdf;let exported;context.window={jspdf:{jsPDF:{API:{autoTable(){}}}}};B.pdf=(data,state)=>{exported={...state};return {save(){}}};await element('br-pdf-current').listeners.click();B.pdf=originalPdf;assert.equal(exported.scope,'full');assert.equal(exported.shs,'s17');
