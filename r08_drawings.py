...
def build(d,g,out):
    ...
    comparisons=''.join(f'<tr><td>{html.escape(o["label"])}</td><td>{o["roll_width_m"]} × {o["roll_length_m"]}</td><td>{o["catalogue_roll_kg"]}</td><td>{o["rolls"]}</td><td>{o["purchase_kg"]:.2f}</td><td>Rp{o["allowance_cost"]:,.0f}</td><td><a href="{html.escape(o["source"])}">Katalog</a></td></tr>' for o in d['mesh_options'])
    ...
    c=d['changes'];page+=f'<div class="card"><b>{d["rhs"]["stock_count"]} batang RHS 6 m</b> · {d["rhs"]["cut_count"]} potongan · {d["rhs"]["length_m"]:.3f} m<br>{len(g["stairs"])} panel tangga tersisa · {c["extra_indicative_seats"]} tambahan tempat duduk indikatif · {c["mesh_area_m2"]:.3f} m² mesh bersih<br><small>RHS R07 101 batang tidak menjadi target R08. Berat berubah mengikuti tebal. SHS tangga + mesh {d["shs"]["stock_count"]} batang; railing lama 20 batang terpisah.</small></div>'
