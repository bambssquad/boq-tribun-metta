...
def build(dist):
 ...
 group_script=s.new_tag('script',src='assets/boq-groups.js');s.select_one('script[src="assets/r04/app.js"]').insert_before(group_script)
 (dist/'assets/boq-groups.js').write_bytes((ROOT/'boq-groups.js').read_bytes())
 group_controls=BeautifulSoup('<div class="group-controls"><label>Pengelompokan <select id="boq-group-mode"><option value="component">Per komponen</option><option value="category">Per kategori biaya</option></select></label><p>Layout mengikuti susunan pekerjaan Excel. Harga dan spesifikasi tetap mengikuti RAB aktif. Profil yang dibeli gabungan tetap satu kelompok agar tidak dihitung dua kali.</p></div>','html.parser')
 s.select_one('#rows').find_parent('div',class_='table-wrap').insert_before(group_controls)
