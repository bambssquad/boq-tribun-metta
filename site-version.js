(()=>{
 'use strict';
 const $=id=>document.getElementById(id),key='metta-site-version-v1';let payload,active,mesh='m20_25',sequence=0;
 const bar=document.createElement('section');bar.id='site-version-bar';bar.setAttribute('aria-label','Versi seluruh desain');
 bar.innerHTML='<strong>VERSI DESAIN</strong><button data-site-version="v1" aria-pressed="false">V1 · Awal</button><button data-site-version="v2" aria-pressed="false">V2 · Infill + mesh</button><p id="site-version-status" role="status">Memuat pilihan model, gambar dan RAB…</p>';
 ...
 const mat=document.createElement('div');mat.className='v2-material site-version-hidden';mat.innerHTML='<h2>04 / Material V2</h2><p>RHS 109 batang 6 m; SHS tangga + mesh 37 batang dan railing 20 batang terpisah. Berat mengikuti tebal di RAB.</p><p>Dek 4 mm; tangga tekuk 3 mm; riser depan tetap pelat 2 mm. Kedua sisi luar memakai mesh, rangka SHS dan strip penjepit. Panel tengah 800 mm diganti infill dengan 20 kolom usulan.</p><a href="#rab-11sep">Buka RAB aktif untuk volume, ketebalan dan pilihan mesh</a>';material.append(mat);
 ...
 function show(version,sync=true){
  if(!payload||!window.MettaModelVersion)return;
  ...
  modelTitle.textContent=newer?'02 / Model koordinasi 3D · V2':oldTitle+' · V1';
  modelNote.textContent=newer?'V2 / R08: infill 800 mm, 45 panel tangga tersisa, 20 kolom tambahan dan sisi mesh terbuka. Model web menunjukkan usulan termasuk penyangga tangga; Revit asli tidak diubah. Sambungan/angkur belum dimodelkan rinci.':oldNote;
  if(hero){hero.src=newer?'assets/r08/01-denah-r08.svg':oldHero.src;hero.alt=newer?'Denah versi V2 / R08':oldHero.alt;}
  captions.forEach(({el,html})=>{el.innerHTML=newer?(el.matches('figcaption')?'<span>V2 / R08 · DENAH INFILL + MESH</span>':'<span><b>100</b> kolom termasuk 20 usulan</span><span><b>45</b> panel tangga</span><span><b>60</b> panel mesh</span>'):html;});
  ...
  $('site-version-status').textContent=newer?'V2 aktif — model 3D, gambar, material dan RAB R08.':'V1 aktif — model dan gambar awal; RAB versi awal.';
  document.documentElement.dataset.siteVersion=version;
  window.dispatchEvent(new CustomEvent('metta:version',{detail:{version}}));
 ...
 fetch('assets/r08/viewer.json').then(r=>{if(!r.ok)throw Error('Model V2 gagal dimuat.');return r.json();}).then(d=>{
  payload=d;let preferred='v2';try{preferred=localStorage.getItem(key)||'v2';}catch{}
