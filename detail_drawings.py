...
def build(dist):
 ...
 g=json.loads((ROOT/'assets/r08/geometry.json').read_text()); native=r04_model.load_model()['items'];d=json.loads((ROOT/'assets/r08/data.json').read_text())
 ...
 def projection(a,items,axes,origin,scale,filter_fn=lambda e:True):
  for e in items:
   ...
   proposed=not str(e['id']).isdigit();fill='#dba64f' if proposed else '#b9ced0'
 ...
 base_g,base_d=g,d
 options=json.loads((ROOT/'assets/r08/options.json').read_text()) if (ROOT/'assets/r08/options.json').exists() else {}
 versions=['v1','v2']+[f'v2-{k}' for k in ('a','c') if k in options and (ROOT/f'assets/r08/geometry-{k}.json').exists()]
 for key in versions:
  version='v1' if key=='v1' else 'v2';g,d=base_g,base_d
  if key not in ('v1','v2'):
   d=options[key[-1]];g=json.loads((ROOT/f'assets/r08/geometry-{key[-1]}.json').read_text())
  sheets=[]
  ...
  (out/f'{key}-01-plan.svg').write_text(plan,encoding='utf-8');add(f'{key}-01-plan.svg','01 · Denah tribun dan akses')
  structural=[e for e in native if e['group'] in ('kolom','balok','stiffener','bracing') and (version=='v1' or e['id'] in g['retained_native_ids'])]
  if version=='v2':structural+=g['frame']+g.get('added_native_items',[])
  a=start(key.upper()+' / 02 · Denah rangka dan jejak kolom')
  projection(a,structural,(0,1),(90,620),.067)
  dim(a,90,90+17700*.067,190,'17700')
  for x,X in ([(4850.0373,5450.0373),(5450.0373,6250.0763),(6250.0763,6850.0751),(12250.0643,12850.0631)] if version=='v1' else [(4850.0373,d['params']['x0']),(d['params']['x0'],d['params']['x1']),(12250.0643,12850.0631)]):dim(a,90+x*.067,90+X*.067,700,str(round(X-x)))
  end(a,f'{key}-02-frame.svg',['Biru: solid luar arsip. Emas: elemen usulan V2. Buka SVG lalu hover elemen untuk ID sumber.',
  ...
  add(f'{key}-02-frame.svg','02 · Denah rangka dengan ID elemen')
  a=start(key.upper()+' / 03 · Tampak rangka depan dan potongan sisi')
  ...
  end(a,f'{key}-03-sections.svg',['Tampak depan adalah proyeksi rangka; komponen di belakang tetap terlihat sebagai gambar koordinasi.',
  ...
  add(f'{key}-03-sections.svg','03 · Tampak rangka dan potongan A–A')
  a=start(key.upper()+' / 04 · Detail simpul — skema koordinasi')
  ...
  end(a,f'{key}-04-joint.svg',['Gambar memperjelas urutan lapisan. Ukuran las dan angkur sengaja tidak ditetapkan tanpa perhitungan.',
  ...
  add(f'{key}-04-joint.svg','04 · Detail simpul dan batas desain')
  if version=='v2':
   for src,title in [('02-infill.svg','05 · Infill, takikan beton dan lapisan'),('03-mesh.svg','06 · Penutup mesh dan penjepit')]:
    name=key+'-'+src;shutil.copy2(ROOT/'assets/r08'/src,out/name);add(name,title)
  ...
  for page in range((len(columns)+23)//24):
   a=start(key.upper()+f' / K{page+1:02} · Jadwal koordinat kolom')
   ...
   for row,e in enumerate(columns[page*24:(page+1)*24]):
    ...
    vals=[str(e['id']),f'{(lo[0]+hi[0])/2:.1f}',f'{(lo[1]+hi[1])/2:.1f}',f'{lo[2]:.1f}',f'{hi[2]:.1f}',f'{hi[2]-lo[2]:.1f}','Usulan' if not str(e['id']).isdigit() else 'Arsip']
    for x,t in zip([45,340,510,680,850,1020,1210],vals):txt(a,x,170+row*24,t,'small')
    line(a,45,178+row*24,1355,178+row*24)
   name=f'{key}-K{page+1:02}-columns.svg'
  ...
  anchors=g.get('structural_anchors',[]) if key not in ('v1','v2') else []
  if anchors:
   a=start(key.upper()+' / S01 · Tumpuan penguatan dan titik angkur usulan')
   projection(a,structural,(0,1),(90,570),.065)
   for anchor in anchors:
    x,y,z=anchor['point'];px,py=90+x*.065,570-y*.065
    a.append(f'<circle cx="{px:.2f}" cy="{py:.2f}" r="6" fill="#ad3737" stroke="white"><title>{html.escape(anchor["id"])}</title></circle>')
   txt(a,45,630,'Titik merah = usulan tumpuan; angka koordinat bukan titik pengeboran final.','small')
   for i,anchor in enumerate(anchors):
    x,y,z=anchor['point'];label=f'{anchor["id"]}: X{x:.1f} / Y{y:.1f} / Z{z:.1f}; {anchor["anchorCount"]} angkur'
    txt(a,45+(i//4)*660,661+(i%4)*26,label,'small')
   target='kolom beton eksisting' if key=='v2-a' else 'lantai / dudukan rangka mandiri'
   name=f'{key}-S01-supports.svg'
   end(a,name,[f'Opsi {key[-1].upper()}: transfer reaksi ke {target}; jumlah angkur adalah allowance RAB.',
    'Kapasitas beton/lantai, tarik, geser, jarak tepi, penanaman dan kelompok angkur belum diverifikasi.',
    'Titik tumpuan FEM merupakan idealisasi. Jangan menetapkan lubang atau fabrikasi dari skema ini.'])
   add(name,'S01 · Titik tumpuan dan angkur penguatan')
  manifest['versions'][key]=sheets
